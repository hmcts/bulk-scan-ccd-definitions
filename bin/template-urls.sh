...
if [[ "$1" == "aat" ]]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
      sed -i '' 's/\${BULK_SCAN_ORCHESTRATOR_URL}/http:\/\/bulk-scan-orchestrator-prod.service.core-compute-prod.internal/g' $FILES
      sed -i '' 's/\${BULK_SCAN_SSCS_URL}/http:\/\/sscs-bulk-scan-prod.service.core-compute-prod.internal/g' $FILES
    else      
      sed -i 's/\${BULK_SCAN_ORCHESTRATOR_URL}/http:\/\/bulk-scan-orchestrator-prod.service.core-compute-prod.internal/g'  $FILES
      sed -i 's/\${BULK_SCAN_SSCS_URL}/http:\/\/sscs-bulk-scan-prod.service.core-compute-prod.internal/g' $FILES
    fi
